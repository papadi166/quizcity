    ...
    <div id="page">
      <table id="cart">
        ...
        <tbody>
          ...
          <tr class="productitm" v-for="item in cart[0].products" :key="item">
            <!-- http://www.inkydeals.com/deal/ginormous-bundle/ -->
            <td><img :src="item.image" class="thumb"></td>
            <td><input @change="cart[0].products['{{item.title}}'].count = value" type="number" :value="item.count" min="1" :max="item.available" class="qtyinput"></td>
            <td>{{item.title}}</td>
            <td>{{item.price * item.count}}$</td>
            <td><span class="remove"><font-awesome-icon :icon="[`fa`, `trash-can`]" size="lg" /></span></td>
          </tr>
          ...
          <tr class="totalprice">
            ...
            <td colspan="2">&nbsp;</td>
            <td colspan="2"><span class="thick">${{cart_total + 4}}</span></td>
          </tr>
